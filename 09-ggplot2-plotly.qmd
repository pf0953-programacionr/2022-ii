...
El siguiente bloque de código, carga y transforma los datos generales de COVID-19 con funciones de `readr` y `dplyr`. La función [read_delim()](https://readr.tidyverse.org/reference/read_delim.html) de [readr](https://readr.tidyverse.org/), lee datos de formatos tabulares (ej. CSV, TSV) y, entre otras ventajas, permite cargar solo las columnas especificadas en el argumento `col_select`.
...
```{r}
#| label: carga-transformacion-datos-covid-cantonales-positivos
#| message: false
...
```{r}
#| label: tabla-covid-cantonales-positivos
#| warning: false
...
Como ejemplo, se crea un [gráfico de dispersión](https://es.wikipedia.org/wiki/Diagrama_de_dispersi%C3%B3n) que muestra la variable *engine displacement* o cilindrada (`displ`) en el eje X, y la variable *highway miles per gallon* o millas por galón en autopista (`hwy`) en el eje Y.
...
##### Temas
`ggplot2` incluye un conjunto de [temas](https://ggplot2.tidyverse.org/reference/ggtheme.html) (estilos) que pueden mejorar el aspecto visual de los gráficos.
...
Otro paquete de temas y recursos relacionados (escalas de colores, fuentes, etc.) es [hrbrthemes](https://github.com/hrbrmstr/hrbrthemes). 
...
Para más información sobre etiquetas, estilos, colores y otros temas relacionados en `ggplot2`, se recomienda leer [ggplot2: Elegant Graphics for Data Analysis - Themes](https://ggplot2-book.org/polishing.html).
...
Si se prefiere el orden inverso, puede utilizarse la función [fct_rev()](https://forcats.tidyverse.org/reference/fct_rev.html) (ej. `fct_rev(fct_infreq(continent))`).
